...
class PongGame:
    ...
    def update(self, state, paddle_v):
        ...
        # paddle position
        paddle_y = max(
            self.half_pad_h, min(self.game_h - self.half_pad_h, paddle_y + paddle_v)
        )
            ...
            if reward != 0:
                ...
                if reward < 0:
                    print(
                        ...
                        % (n_games, n_rewards, n_frames, scores[-1], self.max_score)
                    )
                    n_games += 1
                    if n_games < m_games:
                        scores.append([0])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ball_speed", default="4", help="How fast should the ball be (on average)?"
    )
    parser.add_argument(
        "--paddle_speed", default="8", help="How fast should the paddle be?"
    )
    parser.add_argument("--alpha", default="0.05", help="Reinforcement learning rate")
    parser.add_argument("--epsilon", default="0.1", help="Exploration probability")
